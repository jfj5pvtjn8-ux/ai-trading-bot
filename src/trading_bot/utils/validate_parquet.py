...
- Timestamp alignment (Binance close-time semantics)
- Future timestamps
...
class ParquetValidator:
    ...
    # Timeframe configurations (seconds + expected candles in a "full" file)
    ...
    def validate_file_structure(self, file_path: Path) -> Dict:
        ...
        if not file_path.exists():
            self.errors.append(f"{file_path.name}: File does not exist")
            return result
        ...
        try:
            ...
            if missing_cols:
                self.errors.append(
                    f"{file_path.name}: Missing columns: {sorted(missing_cols)}"
                )
            else:
    ...
    def check_gaps(
        ...
        if len(gaps) > 0:
            for idx in gaps.index:
                ...
                prev_dt = df_sorted.loc[idx - 1, "datetime"]
                curr_dt = df_sorted.loc[idx, "datetime"]
    ...
    def check_duplicates(self, df: pd.DataFrame, file_name: str) -> int:
        ...
        if dup_count > 0:
            ...
            self.errors.append(
                f"{file_name}: {dup_count} duplicate timestamps found "
                f"(examples={list(dup_timestamps[:5])})"
            )
    ...
    def check_ohlcv_integrity(self, df: pd.DataFrame, file_name: str) -> Dict:
        ...
        # High should be >= Low
        issues["high_low_mismatch"] = int((df["high"] < df["low"]).sum())
        ...
        issues["open_close_range"] = int(
            (open_out_of_range | close_out_of_range).sum()
        )
        ...
        issues["zero_ohlc"] = int(all_zero.sum())
    ...
    def check_time_range(self, df: pd.DataFrame, timeframe: str) -> Dict:
        ...
        expected_count = self.TIMEFRAMES.get(timeframe, {}).get("expected", 0)
    ...
    def check_alignment(
        ...
        """
        Check timestamp alignment to timeframe interval (open-time semantics).

        For standard candle open times:
        - timestamp is the candle OPEN time in seconds
        - For interval = tf_seconds:
            aligned  <=>  timestamp % tf_seconds == 0
        """
        if timeframe not in self.TIMEFRAMES:
            return 0, []
        ...
        aligned_mask = df_sorted["timestamp"] % interval == 0
        misaligned = df_sorted.loc[~aligned_mask, "timestamp"]

        count = int(misaligned.shape[0])
        examples = misaligned.head(5).tolist()

        if count > 0:
            self.errors.append(
                f"{file_name}: {count} misaligned timestamps for tf={timeframe}, "
                f"examples={examples}"
            )
    ...
    def check_future_timestamps(
        ...
        """
        Check for candles that appear to be in the future.

        We allow a small tolerance of one interval to account for:
        - slight clock skew
        - the most recent *just closed* candle
        """
        if timeframe not in self.TIMEFRAMES:
            return 0, []
        ...
        df_sorted = df.sort_values("timestamp")
        # Anything more than one interval ahead is "suspicious"
        future_mask = df_sorted["timestamp"] > (now + interval)
        future_ts = df_sorted.loc[future_mask, "timestamp"]

        count = int(future_ts.shape[0])
        examples = future_ts.head(5).tolist()

        if count > 0:
            self.errors.append(
                f"{file_name}: {count} candles appear to be in the future "
                f"(relative to now={now}). Examples: {examples}"
            )
    ...
    def validate_file(self, file_path: Path) -> Dict:
        ...
        if gap_count == 0:
            ...
            for i, gap in enumerate(gap_details[:5], 1):
                print(f"   Gap {i}: {gap['prev_time']} -> {gap['next_time']}")
                print(
                    f"          Missing {gap['missing_candles']} candles "
                    f"({gap['missing_seconds']}s)"
                )
            if len(gap_details) > 5:
                print(f"   ... and {len(gap_details) - 5} more gaps")

        # Alignment (open-time semantics)
        print("\n📏 Timestamp Alignment:")
        misaligned_count, misaligned_examples = self.check_alignment(
            df, timeframe, file_path.name
        )
        if misaligned_count == 0:
            print("   All timestamps correctly aligned: ✓")
        else:
            print(f"   Misaligned timestamps: ✗ {misaligned_count}")
            print(f"   Examples: {misaligned_examples}")

        # Future timestamps
        print("\n⏩ Future Timestamp Check:")
        future_count, future_examples = self.check_future_timestamps(
            df, timeframe, file_path.name
        )
        if future_count == 0:
            print("   No future candles: ✓")
        else:
            print(f"   Candles in the future: ✗ {future_count}")
            print(f"   Examples: {future_examples}")

        # OHLCV integrity
        print("\n💹 OHLCV Integrity:")
        ohlcv_issues = self.check_ohlcv_integrity(df, file_path.name)
        all_good_ohlcv = all(count == 0 for count in ohlcv_issues.values())
        if all_good_ohlcv:
        ...
        result = {
            ...
            "ordered": is_sorted,
            "ohlcv_ok": all_good_ohlcv,
            "alignment_ok": misaligned_count == 0,
            "future_ok": future_count == 0,
    ...
    def validate_all(
        ...
        print(f"\n{'=' * 80}")
        print("PARQUET FILE VALIDATION REPORT")
        print(f"{'=' * 80}")
