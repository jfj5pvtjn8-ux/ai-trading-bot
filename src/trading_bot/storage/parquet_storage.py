...
class ParquetStorage:
    ...
    def __init__(self, base_path: str = "data/live", retention_days: int = 7, fresh_start: bool = True):
        """
        Args:
        ...
        self.executor = ThreadPoolExecutor(max_workers=4, thread_name_prefix="parquet_writer")
    ...
    def save_candle(self, symbol: str, timeframe: str, candle: Dict[str, Any]):
        ...
        Args:
            ...
            candle: Normalized candle dict with ts, open, high, low, close, volume
        """
        try:
            ...
            with file_lock:
                ...
                df_new = pd.DataFrame([{
                    ...
                    'timeframe': timeframe,
                    'timestamp': candle['ts'],
                    'datetime': datetime.fromtimestamp(candle['ts']),
                    'open': candle['open'],
                    'high': candle['high'],
                    'low': candle['low'],
                    'close': candle['close'],
                ...
                if filepath.exists():
                    ...
                    # Create new file
                    df_new.to_parquet(filepath, index=False, compression='snappy')
                    self.logger.info(f"[ParquetStorage] Created new file: {filename}")
    ...
    def save_candles_batch(self, symbol: str, timeframe: str, candles: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            with file_lock:
                ...
                # Prepare dataframe with all candles
                df_new = pd.DataFrame([{
                    'symbol': symbol,
                    'timeframe': timeframe,
                    'timestamp': candle['ts'],
                ...
                self.logger.debug(f"[ParquetStorage] Batch saved {len(candles)} candles to {filename}")
    ...
    def _rotate_to_new_date(self, new_date: str):
        ...
        try:
            ...
            self.logger.error(f"[ParquetStorage] Failed to rotate to new date: {e}")
    ...
    def _delete_todays_files(self):
        ...
        for filepath in self.base_path.glob(pattern):
            try:
                filepath.unlink()
                deleted_count += 1
                self.logger.info(f"[ParquetStorage] Deleted today's file: {filepath.name}")
            except Exception as e:
                self.logger.error(f"[ParquetStorage] Failed to delete {filepath.name}: {e}")
        
        if deleted_count > 0:
            self.logger.info(f"[ParquetStorage] Fresh start: deleted {deleted_count} today's files")
            ...
            try:
                ...
                if len(parts) >= 3:
                    ...
                    if file_date < cutoff_str:
                        filepath.unlink()
                        deleted_count += 1
                        self.logger.debug(f"[ParquetStorage] Deleted old file: {filepath.name}")
            except Exception as e:
                self.logger.error(f"[ParquetStorage] Failed to cleanup {filepath.name}: {e}")
        
        if deleted_count > 0:
            self.logger.info(f"[ParquetStorage] Cleaned up {deleted_count} old files from live")
        ...
        for date_dir in self.archive_path.iterdir():
            ...
            try:
                ...
                if dir_date < cutoff_str:
                    ...
                    self.logger.debug(
                        f"[ParquetStorage] Deleted archive dir: {dir_date} ({file_count} files)"
                    )
            except Exception as e:
                self.logger.error(f"[ParquetStorage] Failed to cleanup archive {date_dir.name}: {e}")
